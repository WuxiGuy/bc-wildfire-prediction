...
class SATDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        assert (
            self.data_path
        ).exists(), "folder prepared_file not found."
    ...
    def get_images(self, index: int, percentile: int = 95):
        ...
        try:
            ...
            image_dict["False Color"] = false_color
            image_dict["Mask"] = mask
            image_dict["NDBI"] = NDBI
    ...
    def get_numerical_values(self, index: int, percentile: int = 95, scale_factor: float = 0.05):
        ...
        try:
            ...
            B11 = self.downsample(self.scale_band(tiff.imread(
                f"{self.data_path.as_posix()}/{filepath}/B11.tif"), percentile=percentile), scale_factor)

            # Resize B11 from (19,19) to (38, 38)
            B11 = cv2.resize(B11, (38, 38), interpolation=cv2.INTER_CUBIC)

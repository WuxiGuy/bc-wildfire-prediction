...
def download_files_for_item_landsat2(
    item: Item, asset_dict: dict[str, str], outpath: Path
) -> bool:
    """
    Save all files of interest for a given item.
    ...
    for key, value in asset_dict.items():
        if key in item.assets:
            ...
            if not file_outpath.exists():
                try:
                    item_content = item.assets[key].to_dict()
                    item_href = item_content['alternate']['s3']['href']
                    print(item_href)
                    download_file(item_href, file_outpath)
                except requests.ConnectionError:
                    print(
                        f"Failed to download {item.assets[key].href} for item {item.id}"
                    )
